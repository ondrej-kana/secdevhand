...
13376_fix_profile_d_timeout_sh() {
    ...
    # Change TMOUT value to 600
    if grep -qE '^\s*TMOUT=' "$file"; then
        sed -i 's/^\s*TMOUT=.*/TMOUT=600/' "$file"
    else
        echo "TMOUT=600" >> "$file"
    fi

    echo "$(date '+%F %T') - Fixed $file: TMOUT set to 600" >> "$GLOBLOGFILE"
}
...
13376_restore_profile_d_timeout_sh() {
    ...
    # Extract TMOUT value from backup
    local tmout_value
    tmout_value=$(grep -E '^\s*TMOUT=' "$backup_file" | awk -F '=' '{print $2}' | xargs)

    if [ -z "$tmout_value" ]; then
        echo "ERROR: TMOUT not found in backup file: $backup_file" >> "$GLOBLOGFILE"
        echo "ERROR: TMOUT not found in backup file: $backup_file"
        return 1
    fi

    # Check if TMOUT exists in current file
    if grep -qE '^\s*TMOUT=' "$file"; then
        # Replace existing TMOUT with backup value
        sed -i "s/^\s*TMOUT=.*/TMOUT=$tmout_value/" "$file"
        echo "$(date '+%F %T') - Restored TMOUT in $file to $tmout_value" >> "$GLOBLOGFILE"
    else
        # If TMOUT line does not exist, append it
        echo "TMOUT=$tmout_value" >> "$file"
        echo "$(date '+%F %T') - TMOUT was missing in $file; added TMOUT=$tmout_value" >> "$GLOBLOGFILE"
    fi
}
