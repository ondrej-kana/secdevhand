...
10492_scan_sysctl_disable_ipv6() {
    ...
    for dir in "${config_dirs[@]}"; do
        ...
        done < <(find "$dir" -maxdepth 1 -name "*.conf" 2>/dev/null)
    done
    # 2.1 Check specificaly /etc/sysctl.conf
    if [ -f /etc/sysctl.conf ] || [ -L /etc/sysctl.conf ]; then
        # Search for settings (Logic retained: [0-1], as only 0 conflicts with 1)
        fs=$(grep -E "^$key\s*=\s*[0-1]" /etc/sysctl.conf | cut -d '=' -f 2- | xargs)
        if [[ -n "$fs" ]]; then
            echo "" >> "$BCKLOGFILE"
            echo "--- START FILE BACKUP: /etc/sysctl.conf ---" >> "$BCKLOGFILE"
            cat /etc/sysctl.conf >> "$BCKLOGFILE"
            echo "--- END FILE BACKUP: /etc/sysctl.conf ---" >> "$BCKLOGFILE"
            echo "" >> "$BCKLOGFILE"
            echo "    Config File Setting: /etc/sysctl.conf ($fs)" >> "$BCKLOGFILE"

            # Check for non-compliant values (any value other than 1)
            if [[ "$fs" != "$required_value" ]]; then
                is_compliant=false
                echo "$(date '+%F %T') - WARNING: /etc/sysctl.conf contains conflicting value $fs." >> "$BCKLOGFILE"
            fi
        fi
    fi
...
10492_fix_sysctl_disable_ipv6() {
    ...
    # 1. Neutralize conflicting files (overwrite value 0 to required_value 1)
    for dir in "${config_dirs[@]}"; do
        #find "$dir" -maxdepth 1 -name "*.conf" 2>/dev/null | while read -r file; do
        while IFS= read -r file; do
            if [ -f "$file" ] || [ -L "$file" ]; then
                ...
                if grep -qE "^$key\s*=\s*0" "$file"; then
                    # sed -i follows symbolic links and modifies the target file
                    sed -i "/^$key\s*=/c\\$key = $required_value" "$file"
                    echo "$(date '+%F %T') - Neutralized conflicting setting in $file to $key = $required_value"
                    changes_made=true
                fi
            fi
        done  < <(find "$dir" -maxdepth 1 -name "*.conf" 2>/dev/null)
    ...
    if [ -f /etc/sysctl.conf ] || [ -L /etc/sysctl.conf ]; then
        if grep -qE "^$key\s*=\s*0" /etc/sysctl.conf; then 
        sed -i "/^$key\s*=/d" /etc/sysctl.conf
        echo "$key = $required_value" >> /etc/sysctl.conf
        echo "$(date '+%F %T') - Updated $key to $required_value in /etc/sysctl.conf (replaced conflicting value 0)." 
        changes_made=true
        fi
    fi
    if ! $changes_made && ! grep -qE "^$key\s*=\s*$required_value(\s|$)" /etc/sysctl.conf; then
        echo "$key = $required_value" >> /etc/sysctl.conf
        echo "$(date '+%F %T') - Added $key = $required_value to /etc/sysctl.conf to ensure permanent compliance."
        changes_made=true
    fi

    # 2. Apply setting to active kernel (by reloading all sysctl files)
    if sysctl --system; then
        echo "$(date '+%F %T') - Successfully applied $key = $required_value to active kernel."
        return 0
    else
        echo "$(date '+%F %T') - ERROR: Failed to apply setting to active kernel via sysctl --system."
        return 1
...
10492_restore_sysctl_disable_ipv6() {
    ...
    # AWK prints $4 (path) and $5 (value in parenthesis), SED removes parentheses
    #grep "Config File Setting:" "$log_file" | awk '{print $4, $5}' | sed 's/[()]//g' > "$restore_list_file"
    sed -n 's/^    Config File Setting: \(.*\) (\(.*\))$/\1 \2/p' "$log_file" > "$restore_list_file"
    ...
    local had_any_cfg=0
    had_any_cfg=$(grep -c "^    Config File Setting:" "$log_file" 2>/dev/null || true)

    if [[ "$had_any_cfg" -eq 0 ]]; then
      if grep -qE "^$key\s*=\s*$required_value\s*$" /etc/sysctl.conf; then
        sed -i "/^$key\s*=\s*$required_value\s*$/d" /etc/sysctl.conf
